...
public class CameraControl : MonoBehaviour {
	...
	private float zoomWaitTime = 1.0f;
	public float orthoSize = 20;
	private bool isMoving = false;
	private bool zoomingOut = false;

    public float m_DampTime = 0.2f;
	public float m_ZoomDampTime = 1.0f;                 // Approximate time for the camera to refocus.
...
//        float requiredSize = FindRequiredSize();
		...
		if (!isMoving && !zoomingOut) {
			zoomWaitTime -= Time.deltaTime;
			if (zoomWaitTime <= 0) {
				Debug.Log ("Zoom out");
				zoomingOut = true;
				zoomWaitTime = 1.0f;
